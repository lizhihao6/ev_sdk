...
int ji_calc_frame(void *predictor, const JI_CV_FRAME *inFrame, const char *args,
    ...
    std::vector<cv::Mat> inMatArray;
    inMatArray.push_back(inMat);

    int processRet = processMat(detector, inMatArray, args, outputFrame, *event);
...
int ji_calc_buffer(void *predictor, const void *buffer, int length, const char *args, const char *outFile,
    ...
    JI_BUFFER_HEADER *bufferHeader = (JI_BUFFER_HEADER*)buffer;
    JI_BUFFER_INFO *bufferInfo = (JI_BUFFER_INFO*)bufferHeader->buffers;
    for(int i = 0 ; i < bufferHeader->nums; i++)
    {
        const unsigned char *b = (const unsigned char *)bufferHeader->buffers[i].buffer;
        std::vector<unsigned char> vecBuffer(b, b + bufferHeader->buffers[i].length);
    ...
    cv::Mat outMat;
    int processRet = processMat(classifierPtr, inMatArray, args, outMat, *event);
...
int ji_calc_video_file(void *predictor, const char *infile, const char* args,
    ...
    while (videoCapture.read(inMat)) {
        ...
        iRet = processMat(classifierPtr, inMatArray, args, outMat, event);
